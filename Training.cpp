...
to represent your school. There are N students for you to pick from. The i-th student has a skill rating Si, 
...
containing the two integers N and P, the number of students and the number of students you need to pick, respectively. 
...
minimum number of hours of coaching needed, before you can pick a fair team of P students.
...
This gives the first, second and third students a skill level of 9. This is the minimum time you can spend, so the answer is 14.
...
In Sample Case #3, P = N, so every student will be on your team. You have to spend 6 hours training the third student, 
so that they have a skill of 7, like everyone else. This is the minimum time you can spend, so the answer is 6.
******/
#include<bits/stdc++.h>
using namespace std;
...
int main()
{
    ...
    for(int i=0;i<t;i++)
    {
        ...
        for(int i=p-1;i<n;i++)
        {
            long t=prefix[i]-prefix[i-(p-1)];
            long final=arr[i]*(p-1)-t;
            ans=min(ans,final);
        }
